...
public:
	...
	inline explicit       Socket(/*sock_type*/int type)
	...
	static std::string      address_string(sockaddr_storage addr);
	static int              discover_mtu(sockaddr_storage remote_address);
	
	// Server initialisation
	void bind(sockaddr_storage local_address,
	          int              max_conn_queue=DEFAULT_MAX_CONN_QUEUE);
	// Client initialisation
	void connect(sockaddr_storage remote_address);
...
Socket* Socket::accept(double timeout_secs) {
	...
	if( ret == -1 && (errno == EAGAIN || errno == EWOULDBLOCK ) ) {
		pollfd pfd = {
			.fd      = _fd,
			.events  = POLLIN | POLLERR | POLLRDNORM,
			.revents = 0
		};
