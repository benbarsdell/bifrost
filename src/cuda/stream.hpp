...
// TODO: Rename scoped_stream?
//         It's a bit like lock_guard vs. unique_lock
class independent_stream : public cuda::stream {
	typedef cuda::stream super_type;
public:
	inline explicit independent_stream(int      priority=0,
	                                   unsigned flags=cudaStreamNonBlocking)
		: super_type(priority, flags) {}
	inline ~independent_stream() { this->synchronize(); }
};
typedef independent_stream scoped_stream; // Alternate name
